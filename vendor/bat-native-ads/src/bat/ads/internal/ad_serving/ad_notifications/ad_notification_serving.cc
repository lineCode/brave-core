...
void AdServing::MaybeServe() {
  ...
  MaybeServeAdForCategories(categories, [this](
      const Result result,
  ...
  database_table.GetForCategories(categories, [=](
    ...
    const CreativeAdNotificationList eligible_ads =
        eligible_ad_notifications.Get(ads,
            last_delivered_creative_ad_notification_, ad_events);
...
void AdServing::MaybeDeliverAd(
  ...
  last_delivered_creative_ad_notification_ = ad;

  ads_->get_client()->UpdateSeenAdvertiser(ad.advertiser_id, 1);
